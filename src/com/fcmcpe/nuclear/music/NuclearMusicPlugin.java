...
public class NuclearMusicPlugin extends PluginBase {
    ...
    public void onEnable() {
        ...
        for (String position : positions) {
            ...
            if (!block.getId().equals(BlockID.NOTEBLOCK)) {
    ...
    static class NodeIntegerPosition {
        int x;
        int y;
        int z;
        Level level;
    ...
    class NuclearMusicListener implements Listener {
        ...
        public void onBlockTouch(PlayerInteractEvent event) {
            ...
            if (!event.getBlock().getId().equals(BlockID.NOTEBLOCK)) return;
            Song song;
            NodeIntegerPosition node = new NodeIntegerPosition(event.getBlock());
            if (!event.getPlayer().hasPermission("nuclearmusic.setup")) {
        ...
        public void onBlockBreak(BlockBreakEvent event) {
            if (event.getBlock().getId().equals(BlockID.NOTEBLOCK)) {
                NodeIntegerPosition node = new NodeIntegerPosition(event.getBlock());
                SongPlayer sp = songPlayers.get(node);
                if (sp != null) {
                    sp.setPlaying(false);
